...
test("完成建议把必做徽标放在标题右侧并让正文另起一行", async () => {
  const source = await readFile(new URL("./main.ts", import.meta.url), "utf8");
  const stylesheet = await readFile(new URL("./styles.css", import.meta.url), "utf8");

  assert.match(
    source,
    /<header class="step-guide-section-head">\s*<span>完成建议<\/span>\s*<strong class="step-guide-requirement"[\s\S]*?<\/strong>\s*<\/header>\s*<p>/,
  );
  assert.doesNotMatch(source, /<p class="step-guide-requirement-line">/);
  assert.match(
    stylesheet,
    /\.step-guide-section-head\s*\{[^}]*display:\s*flex;[^}]*align-items:\s*center;[^}]*gap:\s*7px;/,
  );
  assert.match(
    stylesheet,
    /\.step-guide-section-head \+ p\s*\{[^}]*margin-top:\s*0;/,
  );
});

test("查看提示词使用原脚本只读发送清单而非文本编辑器", async () => {
  const source = await readFile(new URL("./main.ts", import.meta.url), "utf8");
  const action = source.match(/if \(action === "prompt-preview"\) \{([\s\S]*?)\n  \}/)?.[1] ?? "";

  assert.match(action, /openPromptPreview\(preview\)/);
  assert.doesNotMatch(action, /openTextEditor/);
  assert.match(source, /class="prompt-preview-overlay"/);
  assert.match(source, /class="prompt-preview-panel" role="dialog" aria-modal="true"/);
  assert.match(source, /REQUEST MANIFEST/);
  assert.match(source, />本轮发送内容</);
  assert.match(source, /data-prompt-preview-copy/);
  assert.match(source, /data-prompt-preview-close/);
});

test("提示词清单按原脚本顺序显示可折叠消息卡片", async () => {
  const source = await readFile(new URL("./main.ts", import.meta.url), "utf8");

  assert.match(source, /preview\.messages\.map\(\(message, index\) =>/);
  assert.match(source, /class="prompt-message" data-role="\$\{escapeHtml\(message\.role\)\}"/);
  assert.match(source, /data-prompt-message-toggle/);
  assert.match(source, /class="prompt-message-body" hidden/);
  assert.match(source, /classList\.toggle\("is-open"/);
  assert.match(source, /body\.hidden = !open/);
});

test("展开提示词消息不会滚动整个固定弹窗", async () => {
  const source = await readFile(new URL("./main.ts", import.meta.url), "utf8");
  const previewFunction = source.match(/function openPromptPreview[\s\S]*?\n\}\n\nfunction openTextEditor/)?.[0] ?? "";

  assert.doesNotMatch(previewFunction, /item\.scrollIntoView/);
});

test("提示词清单支持关闭键、遮罩、Escape、Android 返回与复制全部", async () => {
  const source = await readFile(new URL("./main.ts", import.meta.url), "utf8");

  assert.match(source, /querySelectorAll\("\[data-prompt-preview-close\]"\)/);
  assert.match(source, /event\.target === overlay/);
  assert.match(source, /event\.key === "Escape"/);
  assert.match(source, /autoCardStudioPromptPreview/);
  assert.match(source, /window\.addEventListener\("popstate"/);
  assert.match(source, /navigator\.clipboard\.writeText/);
});

test("提示词清单复用原脚本面板与消息卡片样式", async () => {
  const stylesheet = await readFile(new URL("./styles.css", import.meta.url), "utf8");

  assert.match(
    stylesheet,
    /\.prompt-preview-overlay\s*\{[^}]*position:\s*fixed;[^}]*inset:\s*0;[^}]*display:\s*grid;[^}]*place-items:\s*center;/,
  );
  assert.match(
    stylesheet,
    /\.prompt-preview-panel\s*\{[^}]*grid-template-rows:\s*auto auto minmax\(0,\s*1fr\);[^}]*width:\s*min\(1080px,\s*calc\(100% - 48px\)\);[^}]*height:\s*min\(780px,\s*calc\(100% - 48px\)\);[^}]*border-radius:\s*18px;/,
  );
  assert.match(
    stylesheet,
    /\.prompt-message\s*\{[^}]*border-left:\s*3px solid var\(--violet\);[^}]*border-radius:\s*10px;[^}]*background:\s*#302e29;/,
  );
  assert.match(
    stylesheet,
    /@media \(max-width: 760px\)[\s\S]*?\.prompt-preview-panel\s*\{[^}]*width:\s*100%;[^}]*height:\s*100%;[^}]*border-radius:\s*14px;/,
  );
  assert.match(
    stylesheet,
    /@media \(max-width: 760px\)[\s\S]*?\.prompt-preview-head\s*\{[^}]*padding-top:\s*max\(22px,\s*calc\(8px \+ env\(safe-area-inset-top,\s*0px\)\)\);/,
  );
});
