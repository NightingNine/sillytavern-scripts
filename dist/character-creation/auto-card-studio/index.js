...
const WORKSPACE_RESIZER_CSS = `
.acs-workspace {
    position: relative;
    --acs-rail-width: max(220px, 16vw);
    --acs-inspector-width: max(300px, 21vw);
    grid-template-columns: var(--acs-rail-width) minmax(360px, 1fr) var(--acs-inspector-width);
}
...
.acs-workspace-resizer:hover::before,
.acs-workspace-resizer:focus-visible::before,
.acs-workspace-resizer.is-dragging::before {
    width: 2px;
    background: var(--acs-cyan);
    box-shadow: 0 0 12px rgba(217, 119, 87, 0.38);
}

.acs-workspace-resizer:focus-visible {
    outline: 2px solid var(--acs-cyan);
    outline-offset: -3px;
}

.acs-workspace-resizer--rail {
    left: var(--acs-rail-width);
    transform: translateX(-50%);
}

.acs-workspace-resizer--inspector {
    right: var(--acs-inspector-width);
    transform: translateX(50%);
}

.acs-shell.is-resizing-workspace,
.acs-shell.is-resizing-workspace * {
    cursor: col-resize !important;
    user-select: none !important;
}

.acs-workspace:has(.acs-inspector.is-expanded) .acs-workspace-resizer {
    visibility: hidden;
}
...
/* 等宽字体保留给英文与数字；中文缺字时回退到正文无衬线字体，避免系统等宽宋体发虚。 */
.acs-shell {
  --acs-mono: "JetBrains Mono", "Cascadia Code", Consolas, var(--acs-body), monospace;
}

/* 分类名称始终使用正文字体；桌面与手机展开导航保持一致。 */
.acs-shell .acs-phase-title {
  font-family: var(--acs-body);
  font-weight: 650;
  letter-spacing: 0.02em;
  text-transform: none;
...
.acs-model-parameter-panel {
  margin-top: -8px;
  margin-bottom: 20px;
  border-color: rgba(183, 163, 207, 0.28);
  background: rgba(183, 163, 207, 0.055);
}
...
const REFERENCE_ASSET_CSS = `
/* 自建正式产物：沿用暖色产物语义，避免与紫色“仅参考”资源混淆。 */
.acs-manual-artifact-create {
  margin-left: auto;
  border-color: rgba(211,173,114,.34);
  color: #dcc193;
}
.acs-manual-artifact-badge {
  flex: 0 0 auto;
  padding: 3px 6px;
  border: 1px solid rgba(211,173,114,.32);
  border-radius: 999px;
  color: #dcc193;
...
.acs-character-project-picker-dialog {
  ...
  overflow: hidden;
  border: 1px solid rgba(211,173,114,.38);
  border-radius: 18px;
  background: #302e29;
  box-shadow: 0 30px 90px rgba(10,9,8,.62);
...
.acs-character-project-picker-dialog > header {
  ...
  border-bottom: 1px solid var(--acs-line-soft);
  background: linear-gradient(120deg,rgba(211,173,114,.1),transparent 58%);
}
...
.acs-manual-artifact-form label,
.acs-reference-worldbook-picker-body label { display: grid; gap: 7px; }
.acs-manual-artifact-form label > span,
.acs-reference-worldbook-picker-body label > span {
  color: var(--acs-muted);
  font: 700 9px/1.3 var(--acs-body);
}
...
.acs-manual-artifact-content-field { min-height: 250px; }
.acs-manual-artifact-content-field textarea {
  ...
  background: #292722;
  color: var(--acs-text);
  font: 450 12px/1.75 var(--acs-body);
  text-rendering: optimizeLegibility;
  -webkit-font-smoothing: antialiased;
}
.acs-manual-artifact-form > p,
.acs-reference-worldbook-picker-body > p {
  margin: 0;
  color: var(--acs-muted);
  font-size: 9px;
  line-height: 1.55;
...
/* 附属世界书：紫色书签导轨是“只参考、永不发布”的稳定视觉标记。 */
.acs-reference-worldbooks-card {
  position: relative;
  display: grid;
  gap: 10px;
  overflow: hidden;
  padding: 12px;
  border: 1px solid rgba(183,163,207,.28);
  border-radius: 11px;
  background: #34312d;
  box-shadow: inset 3px 0 0 rgba(183,163,207,.62);
}
.acs-reference-worldbooks-head {
  display: grid;
  grid-template-columns: 32px minmax(0,1fr) auto;
  gap: 9px;
  align-items: center;
}
.acs-reference-worldbooks-icon,
.acs-reference-worldbook-mark {
  display: grid;
  place-items: center;
  color: #c7b4dc;
...
.acs-reference-worldbooks-head strong,
.acs-reference-worldbooks-head small { display: block; }
.acs-reference-worldbooks-head strong { color: var(--acs-text); font-size: 11px; }
.acs-reference-worldbooks-head small { margin-top: 3px; color: var(--acs-muted); font-size: 8px; }
.acs-reference-worldbooks-head b {
  color: #c7b4dc;
  font: 700 8px/1 var(--acs-body);
  white-space: nowrap;
}
.acs-reference-worldbooks-note {
  margin: 0;
  padding: 8px 9px;
  border: 1px dashed rgba(183,163,207,.24);
  border-radius: 8px;
  color: var(--acs-muted);
  font-size: 9px;
  line-height: 1.5;
}
.acs-reference-worldbooks-note i { margin-right: 5px; color: var(--acs-violet); }
.acs-reference-worldbook-imports { display: grid; grid-template-columns: 1fr 1fr; gap: 7px; }
.acs-reference-worldbook-imports .acs-button { min-width: 0; min-height: 34px; font-size: 9px; }
.acs-reference-worldbook-list { display: grid; gap: 7px; }
.acs-reference-worldbook-empty {
  display: grid;
  justify-items: center;
  gap: 5px;
  padding: 17px 12px;
...
.acs-reference-worldbook-empty i { color: var(--acs-violet); font-size: 17px; }
...
const TEST_BRANCH_BUILD_LABEL = '测试版 2026.08.09-101';
...
const REFERENCE_WORLDBOOK_RESOURCE_KEY = 'reference-worldbooks';
...
.acs-settings-fold{position:relative;margin-bottom:12px;border:1px solid var(--acs-line-soft);border-radius:11px;background:#302e29;overflow:hidden}.acs-settings-fold:has(.acs-styled-select.is-open){z-index:32;overflow:visible}.acs-settings-fold.acs-connection-section{padding:0;border-bottom:1px solid var(--acs-line-soft)}
...
.acs-character-project-options { display:grid; align-content:start; grid-auto-rows:48px; min-height:120px; max-height:min(360px,46vh); overflow:auto; gap:6px; padding-right:3px; scrollbar-width:none; }
.acs-character-project-options::-webkit-scrollbar { display:none; }
.acs-character-project-option { display:grid; grid-template-columns:34px minmax(0,1fr) auto; align-items:center; gap:10px; height:48px; min-height:48px; padding:8px 10px; border:1px solid var(--acs-line-soft); border-radius:10px; background:#34312d; color:var(--acs-text-soft); text-align:left; }
...
.acs-delivery-item.is-release-basis {
  border-color: rgba(183, 163, 207, 0.42);
  background: rgba(183, 163, 207, 0.08);
  cursor: default;
}

.acs-delivery-item.is-release-basis input {
  accent-color: var(--acs-violet);
}
...
@media (max-width: 620px) {
  ...
  .acs-delivery-toolbar { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 10px 12px; }
  .acs-delivery-presets { grid-column: 1 / -1; }
...
.acs-confirm-overlay { position:absolute; inset:0; z-index:110; display:grid; padding:18px; place-items:center; background:rgba(18,16,14,.74); backdrop-filter:blur(9px); }
...
const REORG_PLAN_SCHEMA_VERSION = 3;
...
function createStableReferenceEntryId(entry, index) {
    const storedId = String(entry?.id || '').trim();
    if (/^(?:uid|entry):/.test(storedId)) return storedId;
    const uid = entry?.uid ?? entry?.id ?? entry?.extra?.uid;
    if (uid !== undefined && uid !== null && String(uid).trim()) return `uid:${String(uid).trim()}`;
    const name = String(entry?.name || entry?.comment || '').trim().toLocaleLowerCase();
    return `entry:${index}:${name || 'unnamed'}`;
}

function referenceWorldbookEntriesFromRaw(raw) {
    if (Array.isArray(raw)) return raw;
    if (Array.isArray(raw?.entries)) return raw.entries;
    if (raw?.entries && typeof raw.entries === 'object') return Object.values(raw.entries);
    if (Array.isArray(raw?.data?.entries)) return raw.data.entries;
    if (raw?.data?.entries && typeof raw.data.entries === 'object') return Object.values(raw.data.entries);
    return [];
}
...
function normalizeReferenceEntryActivation(entry) {
    ...
    // v0.6.41 以前的快照没有保存关键词；先保留原来的“全部发送”行为，重新同步后再采用真实规则。
...
function normalizeReferenceWorldbookSnapshot(raw, metadata = {}) {
    ...
    return {
        id: String(metadata.id || globalThis.crypto?.randomUUID?.() || `reference-${Date.now()}-${Math.random().toString(36).slice(2)}`),
        name: String(metadata.name || raw?.name || '未命名附属世界书').trim(),
        sourceType: metadata.sourceType === 'sillytavern' ? 'sillytavern' : 'file',
        sourceWorldbookName: metadata.sourceType === 'sillytavern'
            ? String(metadata.sourceWorldbookName || metadata.name || '').trim()
            : '',
        sourceFileName: String(metadata.sourceFileName || '').trim(),
        entries,
        importedAt: String(metadata.importedAt || now),
        syncedAt: String(metadata.syncedAt || now),
    };
}

function normalizeReferenceWorldbookLibrary(raw) {
    return (Array.isArray(raw) ? raw : []).flatMap(item => {
        try {
            return [normalizeReferenceWorldbookSnapshot(item?.entries || [], {
                ...item,
                id: item?.id,
                name: item?.name,
            ...
            console.warn('[A.U.T.O Card Studio] 已跳过损坏的附属世界书快照。', error);
            return [];
        }
    });
}

function referenceWorldbookProjectState(book, projectData = project, create = false) {
    if (!projectData.referenceWorldbookState || typeof projectData.referenceWorldbookState !== 'object') {
        if (!create) return null;
        projectData.referenceWorldbookState = {};
    }
    let state = projectData.referenceWorldbookState[book.id];
    if (!state && create) {
        state = {
            enabled: false,
            entries: Object.fromEntries(book.entries.map(entry => [entry.id, entry.sourceEnabled !== false])),
        };
        projectData.referenceWorldbookState[book.id] = state;
    }
    return state || null;
}

function normalizeReferenceWorldbookProjectState(raw) {
    const normalized = {};
    if (!raw || typeof raw !== 'object') return normalized;
    for (const [bookId, state] of Object.entries(raw)) {
        if (!state || typeof state !== 'object') continue;
        const entries = {};
        for (const [entryId, enabled] of Object.entries(state.entries || {})) entries[String(entryId)] = enabled === true;
        normalized[String(bookId)] = { enabled: state.enabled === true, entries };
    }
    return normalized;
}

async function persistReferenceWorldbooks() {
...
async function loadStudioResources() {
    const [storedPreset, regexes, referenceWorldbooks] = await Promise.all([
        readResourceRecord('preset'),
        readResourceRecord('regexes'),
        readResourceRecord(REFERENCE_WORLDBOOK_RESOURCE_KEY),
    ]);
...
async function persistAllProjectReferenceStates() {
    const now = new Date().toISOString();
    for (const projectItem of projectLibrary.projects) projectItem.updatedAt = now;
    if (projectDatabaseReady) {
        for (const projectItem of projectLibrary.projects) await persistProjectSnapshot(projectItem);
    } else {
        saveProjectLibrary();
    }
}

async function addReferenceWorldbookSnapshot(raw, metadata) {
    const snapshot = normalizeReferenceWorldbookSnapshot(raw, metadata);
    studioResources.referenceWorldbooks.push(snapshot);
    const state = referenceWorldbookProjectState(snapshot, project, true);
    state.enabled = true;
    await Promise.all([persistReferenceWorldbooks(), persistAllProjectReferenceStates()]);
    renderReferenceWorldbooks();
    renderCurrentStep();
    return snapshot;
}

async function importReferenceWorldbookFile(event) {
    const input = event.currentTarget;
    const file = input.files?.[0];
    if (!file) return;
    try {
        if (file.size > 10 * 1024 * 1024) throw new Error('世界书文件超过 10 MB，无法导入。');
        const raw = JSON.parse(await file.text());
        const name = String(raw?.name || file.name.replace(/\.json$/i, '') || '导入的世界书');
        const snapshot = await addReferenceWorldbookSnapshot(raw, {
            name,
            sourceType: 'file',
            sourceFileName: file.name,
        });
        notify('success', `已导入附属世界书“${snapshot.name}”，并在当前项目启用。`);
    } catch (error) {
        console.error('[A.U.T.O Card Studio] 附属世界书文件导入失败。', error);
        notify('error', error?.message || '附属世界书导入失败，请检查 JSON 文件。');
    } finally {
        input.value = '';
    }
}

function closeReferenceWorldbookPicker() {
...
function openReferenceWorldbookPicker() {
    if (typeof helper?.getWorldbookNames !== 'function' || typeof helper?.getWorldbook !== 'function') {
        notify('error', '当前酒馆助手不支持读取世界书，请更新酒馆助手后重试。');
        return;
    }
    const names = helper.getWorldbookNames().map(value => String(value)).filter(Boolean).sort((a, b) => a.localeCompare(b, 'zh-CN'));
    if (!names.length) {
        notify('warning', 'SillyTavern 中还没有可选择的世界书。');
        return;
    }
    const overlay = shell.querySelector('#acs-reference-worldbook-picker');
...
async function importSelectedReferenceWorldbook() {
    ...
    try {
        const entries = await helper.getWorldbook(sourceName);
        const snapshot = await addReferenceWorldbookSnapshot(entries, {
            name: sourceName,
            sourceType: 'sillytavern',
            sourceWorldbookName: sourceName,
        });
        closeReferenceWorldbookPicker();
        notify('success', `已保存“${snapshot.name}”的独立快照；源世界书后续变化不会自动覆盖。`);
    } catch (error) {
        console.error('[A.U.T.O Card Studio] 读取 SillyTavern 世界书失败。', error);
        notify('error', error?.message || '读取世界书失败。');
    } finally {
        button.disabled = false;
    }
}

async function syncReferenceWorldbook(bookId) {
    const index = studioResources.referenceWorldbooks.findIndex(item => item.id === String(bookId));
    const current = studioResources.referenceWorldbooks[index];
    if (!current || current.sourceType !== 'sillytavern' || !current.sourceWorldbookName) return;
    try {
        const entries = await helper.getWorldbook(current.sourceWorldbookName);
        const refreshed = normalizeReferenceWorldbookSnapshot(entries, {
            ...current,
            id: current.id,
            name: current.name,
            sourceType: 'sillytavern',
            sourceWorldbookName: current.sourceWorldbookName,
            importedAt: current.importedAt,
            syncedAt: new Date().toISOString(),
        });
        const nextEntryIds = new Set(refreshed.entries.map(entry => entry.id));
        for (const projectItem of projectLibrary.projects) {
            const state = referenceWorldbookProjectState(current, projectItem, false);
            if (!state) continue;
            const nextEntries = {};
            for (const entry of refreshed.entries) {
                nextEntries[entry.id] = Object.prototype.hasOwnProperty.call(state.entries || {}, entry.id)
                    ? state.entries[entry.id] === true
                    : entry.sourceEnabled !== false;
            }
            state.entries = nextEntries;
        }
        studioResources.referenceWorldbooks[index] = refreshed;
        await Promise.all([persistReferenceWorldbooks(), persistAllProjectReferenceStates()]);
        renderReferenceWorldbooks();
        renderCurrentStep();
        notify('success', `已重新同步“${current.name}”，共 ${nextEntryIds.size} 个条目。`);
    } catch (error) {
        console.error('[A.U.T.O Card Studio] 附属世界书重新同步失败。', error);
        notify('error', error?.message || '重新同步失败，请确认源世界书仍然存在。');
    }
}

async function deleteReferenceWorldbook(bookId) {
    const book = studioResources.referenceWorldbooks.find(item => item.id === String(bookId));
    if (!book) return;
    const confirmed = await showStudioConfirm({
        title: '从全局资源库删除？',
        message: `“${book.name}”由所有项目共用。删除后，每个项目对它的启用配置也会移除；不会删除 SillyTavern 中的源世界书。`,
        confirmLabel: '删除全局快照',
        danger: true,
    });
    if (!confirmed) return;
    studioResources.referenceWorldbooks = studioResources.referenceWorldbooks.filter(item => item.id !== book.id);
    for (const projectItem of projectLibrary.projects) delete projectItem.referenceWorldbookState?.[book.id];
    await Promise.all([persistReferenceWorldbooks(), persistAllProjectReferenceStates()]);
    renderReferenceWorldbooks();
    renderCurrentStep();
...
function renderReferenceWorldbooks() {
    ...
    const books = studioResources.referenceWorldbooks || [];
    const enabledBooks = books.filter(book => referenceWorldbookProjectState(book, project, false)?.enabled);
    summary.textContent = books.length ? `${enabledBooks.length} / ${books.length} 本启用` : '尚未导入';
    list.replaceChildren();
    if (!books.length) {
        const empty = document.createElement('div');
        empty.className = 'acs-reference-worldbook-empty';
        empty.innerHTML = '<i class="fa-regular fa-bookmark" aria-hidden="true"></i><strong>还没有附属世界书</strong><span>导入后只作为 AI 生成资料，不会进入产物库或最终角色卡。</span>';
        list.append(empty);
        return;
    }
...
function normalizeProject(saved) {
    ...
    const normalized = {
        ...
        conversationShieldVersionIds: Array.isArray(saved.conversationShieldVersionIds)
            ...
            : [],
        referenceWorldbookState: normalizeReferenceWorldbookProjectState(saved.referenceWorldbookState),
        ui: {
            ...clean.ui,
            ...(saved.ui || {}),
    ...
    // 全局附属世界书快照只在导入时合并到资源库，不作为项目元数据重复保存。
    delete normalized.referenceWorldbooks;
...
function loadModelParameterSettings(fallbackConnection = {}) {
    try {
        const saved = JSON.parse(localStorage.getItem(MODEL_PARAMETERS_STORAGE_KEY));
        if (saved && typeof saved === 'object') {
            return {
                values: normalizeModelParameters(saved.values || {}),
                customized: saved.customized === true,
            };
        }
    } catch (error) {
        console.warn('[A.U.T.O Card Studio] 无法读取独立模型参数，将尝试迁移旧连接参数。', error);
    }
    // 首次升级时迁移旧版连接设置中的参数，避免已经调整过的数值丢失。
    return {
        values: normalizeModelParameters(fallbackConnection.modelParameters || {}),
        customized: fallbackConnection.modelParametersCustomized === true,
    };
}

function saveModelParameterSettings() {
    localStorage.setItem(MODEL_PARAMETERS_STORAGE_KEY, JSON.stringify(modelParameterSettings));
}
...
function artifactDisplayName(tag, stepNumber, version = null) {
    if (version?.source === 'manual' && String(version.displayName || '').trim()) {
        return String(version.displayName).trim();
    }
...
function artifactGroupMatchesFilter(group) {
    ...
    const haystack = `${artifactDisplayName(group.tag, artifact.step, artifact)} ${group.tag} ${step?.name || ''} step ${artifact.step} s${String(artifact.step).padStart(2, '0')}`
        .toLocaleLowerCase();
...
function renderArtifacts() {
    ...
    if (help) help.textContent = '这里包含 A.U.T.O 生成产物与自建正式产物；切换到哪一版，后续设计与发布就使用哪一版。';
    ...
    for (const [groupIndex, group] of renderedArtifactGroups.entries()) {
        ...
        name.textContent = artifactDisplayName(group.tag, artifact.step, artifact);
        ...
        title.prepend(contextToggle);
        title.append(name);
        if (artifact.source === 'manual') {
            const badge = document.createElement('span');
            badge.className = 'acs-manual-artifact-badge';
            badge.textContent = '自建';
            title.append(badge);
        }
        ...
        if (artifact.source === 'manual') {
            const rename = document.createElement('button');
            rename.type = 'button';
            rename.className = 'acs-artifact-action';
            rename.dataset.editManualArtifact = artifact.id;
            rename.title = '修改自建产物名称与正文';
            rename.innerHTML = '<i class="fa-solid fa-pen" aria-hidden="true"></i><span>修改</span>';
...
function closeManualArtifactDialog() {
    const overlay = shell?.querySelector('#acs-manual-artifact-overlay');
    if (!overlay) return;
    overlay.hidden = true;
    overlay.setAttribute('aria-hidden', 'true');
    overlay.dataset.artifactId = '';
}

function openManualArtifactDialog(artifactId = '') {
    const overlay = shell.querySelector('#acs-manual-artifact-overlay');
    const stored = artifactId
        ? artifactVaultFor(project.id).versions.find(item => item.id === String(artifactId) && item.source === 'manual')
        : null;
    ...
    overlay.hidden = false;
    overlay.setAttribute('aria-hidden', 'false');
    overlay.querySelector('#acs-manual-artifact-name').focus({ preventScroll: true });
}

async function saveManualArtifact() {
    const overlay = shell.querySelector('#acs-manual-artifact-overlay');
    const name = overlay.querySelector('#acs-manual-artifact-name').value.trim();
    const content = overlay.querySelector('#acs-manual-artifact-content').value.trim();
    const stepNumber = Number(overlay.querySelector('#acs-manual-artifact-step').value);
    if (!name) {
        notify('warning', '请填写产物名称。');
        overlay.querySelector('#acs-manual-artifact-name').focus();
        return;
    }
    if (!content) {
        notify('warning', '请填写产物正文。');
        overlay.querySelector('#acs-manual-artifact-content').focus();
        return;
    }
    if (!STEPS.some(step => step.number === stepNumber)) {
        notify('error', '请选择有效的所属步骤。');
        return;
    }

    const vault = artifactVaultFor(project.id);
    const existing = vault.versions.find(item => (
        item.id === String(overlay.dataset.artifactId || '') && item.source === 'manual'
    ));
    const now = new Date().toISOString();
    if (existing) {
        ...
        const id = globalThis.crypto?.randomUUID?.() || `manual-${Date.now()}-${Math.random().toString(36).slice(2)}`;
        const identity = `MANUAL_${String(id).replace(/[^A-Za-z0-9_]/g, '_')}`;
        const version = {
            id,
            step: stepNumber,
            identity,
    ...
    vault.updatedAt = now;
    const stepState = project.steps[existing?.step || stepNumber];
    if (stepState?.status === 'idle') stepState.status = 'draft';
    if (stepState) stepState.updatedAt = now;
    saveProject();
    await persistArtifactVault(project.id);
    closeManualArtifactDialog();
    renderArtifacts();
    renderStepRail();
    renderProgress();
    renderCurrentStep();
    notify('success', existing ? `已保存自建产物“${name}”。` : `“${name}”已作为正式产物存入。`);
}
...
async function deleteArtifact(button) {
    ...
    const displayName = artifactDisplayName(group.tag, latest.step, latest);
    if (!await showStudioConfirm({
        title: '删除产物？',
...
function workspacePanelLimits(side) {
    const workspace = shell.querySelector('.acs-workspace');
    const other = shell.querySelector(side === 'rail' ? '.acs-inspector' : '.acs-rail');
    const minimum = side === 'rail' ? 180 : 250;
    const fixedMaximum = side === 'rail' ? 520 : 720;
    const availableMaximum = workspace.clientWidth > 0
        ? workspace.clientWidth - other.offsetWidth - 360
        : fixedMaximum;
    return { minimum, maximum: Math.max(minimum, Math.min(fixedMaximum, availableMaximum)) };
}

function setWorkspacePanelWidth(side, width, persist = false) {
    const workspace = shell.querySelector('.acs-workspace');
    const { minimum, maximum } = workspacePanelLimits(side);
    const normalized = Math.round(Math.max(minimum, Math.min(maximum, Number(width) || minimum)));
    workspace.style.setProperty(side === 'rail' ? '--acs-rail-width' : '--acs-inspector-width', `${normalized}px`);
    const handle = shell.querySelector(`[data-workspace-resizer="${side}"]`);
    handle?.setAttribute('aria-valuenow', String(normalized));
    handle?.setAttribute('aria-valuemin', String(minimum));
    handle?.setAttribute('aria-valuemax', String(maximum));
    if (persist) saveWorkspaceWidths();
}

function saveWorkspaceWidths() {
    localStorage.setItem(WORKSPACE_WIDTHS_KEY, JSON.stringify({
        rail: Math.round(shell.querySelector('.acs-rail').offsetWidth),
        inspector: Math.round(shell.querySelector('.acs-inspector').offsetWidth),
    }));
}

function restoreWorkspaceWidths() {
    try {
        const stored = JSON.parse(localStorage.getItem(WORKSPACE_WIDTHS_KEY) || 'null');
        const workspace = shell.querySelector('.acs-workspace');
        if (Number.isFinite(stored?.rail)) {
            workspace.style.setProperty('--acs-rail-width', `${Math.max(180, Math.min(520, stored.rail))}px`);
        }
        if (Number.isFinite(stored?.inspector)) {
            workspace.style.setProperty('--acs-inspector-width', `${Math.max(250, Math.min(720, stored.inspector))}px`);
        }
    } catch (error) {
        console.warn('[A.U.T.O Card Studio] 读取工作区宽度失败，将使用默认布局。', error);
    }
}

function syncWorkspaceResizeHandles() {
    for (const side of ['rail', 'inspector']) {
        const panel = shell?.querySelector(side === 'rail' ? '.acs-rail' : '.acs-inspector');
        const handle = shell?.querySelector(`[data-workspace-resizer="${side}"]`);
        if (!panel || !handle || panel.offsetWidth <= 0) continue;
        const { minimum, maximum } = workspacePanelLimits(side);
...
function fitWorkspaceWidthsToViewport() {
    const workspace = shell?.querySelector('.acs-workspace');
    const rail = shell?.querySelector('.acs-rail');
    const inspector = shell?.querySelector('.acs-inspector');
    if (!workspace || !rail || !inspector || workspace.clientWidth <= 0) return;
    if (rail.offsetWidth + inspector.offsetWidth + 360 <= workspace.clientWidth) {
        syncWorkspaceResizeHandles();
        return;
    }
    // 优先收拢右侧，再校正左侧，确保中央创作区始终保留可用宽度。
    setWorkspacePanelWidth('inspector', inspector.offsetWidth);
...
function installWorkspaceResizers() {
    ...
    for (const side of ['rail', 'inspector']) {
        const label = side === 'rail' ? '调整左侧步骤栏宽度' : '调整右侧检查器宽度';
        const handle = document.createElement('div');
        handle.className = `acs-workspace-resizer acs-workspace-resizer--${side}`;
        handle.dataset.workspaceResizer = side;
        handle.tabIndex = 0;
        handle.setAttribute('role', 'separator');
        handle.setAttribute('aria-orientation', 'vertical');
        handle.setAttribute('aria-label', label);
        handle.title = `${label}；双击恢复默认宽度`;
        ...
        handle.addEventListener('pointerdown', event => {
            ...
            const move = moveEvent => {
                ...
                setWorkspacePanelWidth(side, startWidth + delta * direction);
            };
        ...
        handle.addEventListener('keydown', event => {
            ...
            setWorkspacePanelWidth(side, panel.offsetWidth + (event.key === 'ArrowRight' ? 1 : -1) * direction * step, true);
        });
        handle.addEventListener('dblclick', () => {
            workspace.style.removeProperty('--acs-rail-width');
...
function installStudioToolsUI() {
    ...
    if (!shell.querySelector('#acs-new-manual-artifact')) {
        const createButton = document.createElement('button');
        createButton.id = 'acs-new-manual-artifact';
        createButton.className = 'acs-inspector-action acs-manual-artifact-create';
        createButton.type = 'button';
        createButton.title = '手动创建一项正式产物';
        createButton.innerHTML = '<i class="fa-solid fa-plus" aria-hidden="true"></i><span>自建产物</span>';
        shell.querySelector('#acs-expand-artifacts').before(createButton);

        const overlay = document.createElement('div');
        overlay.id = 'acs-manual-artifact-overlay';
        overlay.className = 'acs-manual-artifact-overlay';
        overlay.hidden = true;
        overlay.setAttribute('aria-hidden', 'true');
        overlay.innerHTML = `
          <section class="acs-manual-artifact-dialog" role="dialog" aria-modal="true" aria-labelledby="acs-manual-artifact-title">
            <header class="acs-manual-artifact-head">
              <div><p>FORMAL ARTIFACT</p><h2 id="acs-manual-artifact-title">新建自建产物</h2></div>
              <button class="acs-resource-editor-close" type="button" data-manual-artifact-close aria-label="关闭"><i class="fa-solid fa-xmark"></i></button>
            </header>
            <div class="acs-manual-artifact-form">
              <label><span>所属步骤 <b>必选</b></span><select id="acs-manual-artifact-step"></select></label>
              <label><span>产物名称</span><input id="acs-manual-artifact-name" type="text" maxlength="80" placeholder="例如：港区势力补充设定"></label>
              <label class="acs-manual-artifact-content-field"><span>产物正文</span><textarea id="acs-manual-artifact-content" spellcheck="false" placeholder="填写要进入后续设计与最终角色卡的正式内容。"></textarea></label>
              <p><i class="fa-solid fa-box-archive" aria-hidden="true"></i> 自建产物会进入后续上下文与最终发布；内部身份标签由创作台自动管理。</p>
            </div>
            <footer class="acs-manual-artifact-actions">
              <button class="acs-button" type="button" data-manual-artifact-close>取消</button>
              <button id="acs-save-manual-artifact" class="acs-button acs-button-publish" type="button"><i class="fa-solid fa-floppy-disk"></i><span>存入产物库</span></button>
            </footer>
          </section>`;
        shell.append(overlay);
        createButton.addEventListener('click', () => openManualArtifactDialog());
        overlay.addEventListener('click', event => {
            if (event.target === overlay || event.target.closest('[data-manual-artifact-close]')) closeManualArtifactDialog();
        });
        overlay.addEventListener('keydown', event => {
            ...
            if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's') {
                event.preventDefault();
                void saveManualArtifact();
            }
        });
        overlay.querySelector('#acs-save-manual-artifact').addEventListener('click', () => { void saveManualArtifact(); });
    }
}
...
function renderModelParameterControls() {
    ...
    const parameters = ensureModelParameters();
    for (const [key] of MODEL_PARAMETER_FIELDS) {
...
function updateModelParameter(event) {
    ...
    const parameters = normalizeModelParameters(modelParameterSettings.values || {});
    ...
    modelParameterSettings.values = parameters;
    modelParameterSettings.customized = true;
    saveModelParameterSettings();
    renderModelParameterControls();
}

function resetModelParametersToPreset() {
    ensureModelParameters(studioResources.preset, true);
    renderModelParameterControls();
...
function installModelParameterUI() {
    const connectionSection = shell.querySelector('.acs-connection-section');
    if (!connectionSection || shell.querySelector('#acs-model-parameter-panel')) return;
    const panel = document.createElement('div');
    panel.id = 'acs-model-parameter-panel';
    panel.className = 'acs-connection-profile-panel acs-model-parameter-panel is-collapsed';
    panel.innerHTML = `
      <div class="acs-connection-profile-heading">
        <button id="acs-model-parameter-toggle" class="acs-connection-profile-toggle" type="button" aria-expanded="false" aria-controls="acs-model-parameter-body">
          <span>模型参数</span>
          <span>
            <small>独立设置</small>
            <i class="fa-solid fa-chevron-down" aria-hidden="true"></i>
      ...
      <div id="acs-model-parameter-body" class="acs-connection-profile-body">
...
function buildReferenceWorldbookContext(userInput = '') {
    ...
    let previousBookId = '';
    for (const { book, entry } of enabledEntries) {
        if (book.id !== previousBookId) {
            sections.push(`\n# 参考世界书：${book.name}`);
    ...
    if (!reorgOnly) {
        ...
        if (referenceContextContent) {
            const referenceContext = {
                role: 'user',
                content: prepareTemplateMacrosForGeneration(referenceContextContent),
            };
            if (includePreviewMetadata) referenceContext.name = '附属世界书参考资料（不参与发布）';
            ordered.push(referenceContext);
        }
    }
...
async function runStepGeneration(step, state, userInput, {
    ...
    try {
        ...
        // 用户消息写入后立即显示，不等待流式占位或完整 AI 回复返回。
        renderCurrentStep();
...
function deliveryTargetForArtifact(tag, stepNumber) {
    ...
    const exactTargets = {
        ...
        reorg_plan: '发布依据 · 世界书重组方案',
        opening: '角色卡 · 其他开场',
    };
    if (exactTargets[rawTag]) {
        const kind = rawTag === 'opening' ? 'opening' : rawTag === 'reorg_plan' ? 'reorg_plan' : 'worldbook';
        return { kind, name: exactTargets[rawTag] };
    }
    ...
    if (rawTag.startsWith('schema_') || rawTag === 'schema') {
        const suffix = rawTag.startsWith('schema_') ? artifactSuffixLabel(rawTag.slice('schema_'.length)) : '';
        return { kind: 'worldbook', name: `🗑️变量结构定义${suffix ? ` · ${suffix}` : ''}` };
    }
    if (rawTag.startsWith('SYS_task_')) {
...
function collectDeliveryArtifacts() {
    return collectArtifactGroups().flatMap(group => {
        const artifact = selectedArtifactForGroup(group);
        const displayName = artifactDisplayName(group.tag, artifact.step, artifact);
        // 自建产物是正式世界书内容；用醒目的启用前缀进入发布，但不要求用户理解内部标签。
        const target = artifact.source === 'manual'
            ? { kind: 'worldbook', name: `🧩${displayName}` }
            : deliveryTargetForArtifact(group.tag, artifact.step);
        if (!target) return [];
        return [{
...
function currentReorgArtifacts() {
    // 报告覆盖当前全部正式世界书产物；发布时可以从完整方案中安全裁剪本次勾选的子集。
    return collectDeliveryArtifacts().filter(item => item?.target?.kind === 'worldbook');
}
...
function latestReorgPlanResult() {
    ...
    try {
        ...
        if (!Array.isArray(plan?.mappings)
            || (!plan.mappings.length && !(Array.isArray(plan.discardedBlockIds) && plan.discardedBlockIds.length))) {
            throw new Error('mappings 与 discardedBlockIds 缺失或同时为空');
        }
...
function validateReorgPlan(plan, selectedArtifacts) {
    ...
    if (!Array.isArray(plan.mappings)) {
        addError('mappings', '必须是数组');
    } else if (!plan.mappings.length && !(Array.isArray(plan.discardedBlockIds) && plan.discardedBlockIds.length)) {
        addError('mappings', 'mappings 与 discardedBlockIds 不能同时为空');
    }
    if (plan.blockActions !== undefined && !Array.isArray(plan.blockActions)) {
        addError('blockActions', '必须是数组');
    }

    const discardedBlockIds = new Set();
    if (plan.discardedBlockIds !== undefined && !Array.isArray(plan.discardedBlockIds)) {
        addError('discardedBlockIds', '必须是数组');
    }
    for (const blockId of (Array.isArray(plan.discardedBlockIds) ? plan.discardedBlockIds : [])) {
        if (typeof blockId !== 'string' || !expectedBlockIds.has(blockId)) {
            addError('discardedBlockIds', `包含不存在的 blockId（${String(blockId)}）`);
        } else if (discardedBlockIds.has(blockId)) {
            addError('discardedBlockIds', `${blockId} 被重复声明为废弃`);
        } else {
            discardedBlockIds.add(blockId);
        }
    }
        ...
        if (!Array.isArray(mapping.blockIds) || !mapping.blockIds.length) {
            ...
            for (const blockId of mapping.blockIds) {
                if (typeof blockId !== 'string' || !expectedBlockIds.has(blockId)) {
                    ...
                    addError(`${path}.blockIds`, `${blockId} 已声明废弃，不能同时写入条目`);
    ...
    return { valid: errors.length === 0, errors, warnings, model, discardedBlockIds };
}
...
function filterReorgPlanForArtifacts(plan, selectedArtifacts) {
    ...
    return {
        ...plan,
        blockActions: (Array.isArray(plan.blockActions) ? plan.blockActions : [])
            .filter(action => selectedBlockIds.has(action?.blockId)),
        mappings: (Array.isArray(plan.mappings) ? plan.mappings : [])
            .map(mapping => ({
                ...mapping,
                blockIds: (Array.isArray(mapping?.blockIds) ? mapping.blockIds : [])
                    .filter(blockId => selectedBlockIds.has(blockId)),
            }))
            .filter(mapping => mapping.blockIds.length),
        discardedBlockIds: (Array.isArray(plan.discardedBlockIds) ? plan.discardedBlockIds : [])
            .filter(blockId => selectedBlockIds.has(blockId)),
    };
}
...
function applyReorgPlan(selectedArtifacts, allArtifacts, planResult) {
    ...
    const effectivePlan = filterReorgPlanForArtifacts(planResult.plan, selectedWorldbook);
    const validation = validateReorgPlan(effectivePlan, selectedWorldbook);
    if (!validation.valid) {
        return {
            applied: false,
    ...
    const actionMap = new Map((effectivePlan.blockActions || []).map(action => [action.blockId, action]));
    const usedArtifactIds = new Set();
    const discardedArtifactIds = new Set([...validation.discardedBlockIds]
        .map(blockId => model.blockById.get(blockId)?.artifact?.id)
        .filter(Boolean));
    const unresolvedBlockIds = [];
    const mappings = effectivePlan.mappings.map((mapping, sourceIndex) => ({ ...mapping, sourceIndex, actionMap }));
    ...
    if (!resolvedMappings.length) {
        if (discardedArtifactIds.size === selectedWorldbook.length) {
            return {
                applied: true,
                entries: [],
                plan: effectivePlan,
                usedArtifacts: 0,
                handledArtifacts: discardedArtifactIds.size,
                usedArtifactIds: [],
                discardedArtifacts: discardedArtifactIds.size,
                discardedArtifactIds: [...discardedArtifactIds],
                omittedArtifacts: 0,
                unresolvedBlockIds: [],
                validationWarnings: validation.warnings,
            };
    ...
    const handledArtifactIds = new Set([...usedArtifactIds, ...discardedArtifactIds]);
    return {
        applied: true,
        entries,
        plan: effectivePlan,
        usedArtifacts: usedArtifactIds.size,
        handledArtifacts: handledArtifactIds.size,
        usedArtifactIds: [...usedArtifactIds],
        discardedArtifacts: discardedArtifactIds.size,
        discardedArtifactIds: [...discardedArtifactIds],
        omittedArtifacts: Math.max(0, selectedWorldbook.length - handledArtifactIds.size),
...
function isCompleteReorgBuild(build, selectedArtifacts) {
    ...
    return Boolean(
        build?.applied
        && (build.handledArtifacts ?? build.usedArtifacts) === selectedWorldbookCount
...
async function ensureDeliveryReorg(selectedArtifacts) {
    ...
    if (!cachedReorgCoversSelection(selectedWorldbook)) {
        throw new Error('本次勾选中包含 Step 29 分析报告之外的产物，请返回“世界书重组”重新分析并生成方案');
    }
...
function cachedReorgCoversSelection(selectedArtifacts) {
    if (project.autoReorg?.schemaVersion !== REORG_PLAN_SCHEMA_VERSION || !project.autoReorg?.selectionSignature) return false;
    try {
        const analyzed = new Set(JSON.parse(project.autoReorg.selectionSignature).map(item => JSON.stringify(item)));
        return JSON.parse(reorgSelectionSignature(selectedArtifacts)).every(item => analyzed.has(JSON.stringify(item)));
    } catch {
        return false;
    }
}
...
function renderDeliveryReorgStatus() {
    ...
    if (result.status === 'ready' && !cachedReorgCoversSelection(currentReorgArtifacts())) {
        element.classList.add('is-warning');
        element.textContent = 'Step 29 重组方案已过期：当前产物或方案格式已变化，请重新分析并生成';
        return;
    }
    if (result.status === 'ready') {
        element.classList.add('is-active');
        const discarded = Array.isArray(result.plan.discardedBlockIds) ? result.plan.discardedBlockIds.length : 0;
        element.textContent = `已有 Step 29 重组方案：${result.plan.mappings.length} 项映射${discarded ? ` · ${discarded} 项废弃` : ''}`;
...
function renderDeliveryArtifacts() {
    ...
    for (const artifact of deliveryArtifacts) {
        const isReleaseBasis = artifact.target.kind === 'reorg_plan';
        const item = document.createElement('label');
        item.className = `acs-delivery-item${artifact.accepted ? '' : ' is-draft'}${isReleaseBasis ? ' is-release-basis' : ''}`;
        const input = document.createElement('input');
        input.type = 'checkbox';
        input.value = artifact.id;
        input.checked = isReleaseBasis || artifact.accepted;
        input.disabled = isReleaseBasis;
        if (isReleaseBasis) input.setAttribute('aria-label', '世界书重组方案是本次发布的固定依据');
        ...
        meta.innerHTML = `<span>${isReleaseBasis ? '固定依据' : artifact.accepted ? '已确认' : '草案'}</span><span>S${String(artifact.step).padStart(2, '0')}</span>`;
...
function referenceWorldbookBundleForProject(projectData = project) {
    const referencedIds = new Set(Object.keys(projectData.referenceWorldbookState || {}));
    return (studioResources.referenceWorldbooks || [])
        .filter(book => referencedIds.has(book.id))
        .map(book => JSON.parse(JSON.stringify(book)));
}

async function mergeImportedReferenceWorldbooks(rawBooks, importedProject) {
    const importedBooks = normalizeReferenceWorldbookLibrary(rawBooks);
    if (!importedBooks.length) return;
    const idRemap = new Map();
    let changed = false;
    for (const importedBook of importedBooks) {
        const existing = studioResources.referenceWorldbooks.find(item => item.id === importedBook.id);
        if (!existing) {
            studioResources.referenceWorldbooks.push(importedBook);
            idRemap.set(importedBook.id, importedBook.id);
            changed = true;
            continue;
        }
        const sameSnapshot = JSON.stringify(existing.entries) === JSON.stringify(importedBook.entries)
            && existing.name === importedBook.name;
        if (sameSnapshot) {
            idRemap.set(importedBook.id, existing.id);
            continue;
        }
        const replacementId = globalThis.crypto?.randomUUID?.() || `reference-${Date.now()}-${Math.random().toString(36).slice(2)}`;
        studioResources.referenceWorldbooks.push({ ...importedBook, id: replacementId });
        idRemap.set(importedBook.id, replacementId);
        changed = true;
    }
    const remappedState = {};
    for (const [bookId, state] of Object.entries(importedProject.referenceWorldbookState || {})) {
        remappedState[idRemap.get(bookId) || bookId] = state;
    }
    importedProject.referenceWorldbookState = remappedState;
    if (changed) await persistReferenceWorldbooks();
}
...
async function exportProjectJson() {
    ...
    downloadBlob(
        JSON.stringify({
            ...project,
            artifactVault: vault,
            referenceWorldbooks: referenceWorldbookBundleForProject(project),
        }, null, 2),
...
async function importProjectJson(event) {
    ...
    try {
        ...
        if (!studioResources.loaded) await loadStudioResources();
        await mergeImportedReferenceWorldbooks(rawProject?.referenceWorldbooks, imported);
...
function installSettingsCollapsibles() {
    ...
    const resources = createSection('resources', '创作资源', '独立预设、正则与参考世界书');
    ...
    const referenceCard = flowFields?.querySelector('.acs-reference-worldbooks-card');
    if (presetCard) resourceStack.append(presetCard);
    if (regexCard) resourceStack.append(regexCard);
    if (referenceCard) resourceStack.append(referenceCard);
...
function installResourceManagerUI() {
    ...
    referenceCard.className = 'acs-reference-worldbooks-card';
    referenceCard.innerHTML = `
      <div class="acs-reference-worldbooks-head">
        <span class="acs-reference-worldbooks-icon"><i class="fa-solid fa-book-bookmark" aria-hidden="true"></i></span>
      ...
      <div id="acs-reference-worldbook-list" class="acs-reference-worldbook-list"></div>`;
    regexCard.after(referenceCard);
    ...
    const referenceInput = presetInput.cloneNode();
    referenceInput.id = 'acs-import-reference-worldbook-file';
    shell.append(presetInput, regexInput, referenceInput);
    ...
    referencePicker.id = 'acs-reference-worldbook-picker';
    referencePicker.className = 'acs-reference-worldbook-picker';
    referencePicker.hidden = true;
    referencePicker.setAttribute('aria-hidden', 'true');
    referencePicker.innerHTML = `
      <section class="acs-reference-worldbook-picker-dialog" role="dialog" aria-modal="true" aria-labelledby="acs-reference-worldbook-picker-title">
        <header>
          <div><p>SNAPSHOT IMPORT</p><h2 id="acs-reference-worldbook-picker-title">选择 SillyTavern 世界书</h2></div>
        ...
        <footer>
          <button class="acs-button" type="button" data-reference-picker-close>取消</button>
          <button id="acs-import-selected-reference-worldbook" class="acs-button acs-button-publish" type="button"><i class="fa-solid fa-camera"></i>保存快照</button>
        </footer>
      </section>`;
    ...
    referencePicker.addEventListener('click', event => {
        if (event.target === referencePicker || event.target.closest('[data-reference-picker-close]')) closeReferenceWorldbookPicker();
    });
    referencePicker.addEventListener('keydown', event => {
        if (event.key === 'Escape') {
            event.stopPropagation();
            closeReferenceWorldbookPicker();
        }
    ...
    referenceCard.addEventListener('click', event => {
        ...
        if (switchControl) {
            // summary 内的开关不能顺带展开卡片；统一手动切换并触发 change。
            event.preventDefault();
            event.stopPropagation();
        ...
        if (syncButton) {
            event.preventDefault();
            void syncReferenceWorldbook(syncButton.dataset.referenceBookSync);
            return;
        }
        const deleteButton = event.target.closest('[data-reference-book-delete]');
        if (deleteButton) {
            event.preventDefault();
            void deleteReferenceWorldbook(deleteButton.dataset.referenceBookDelete);
        }
    });
    referenceCard.addEventListener('change', event => {
        const bookToggle = event.target.closest('[data-reference-book-toggle]');
        if (bookToggle) {
            const book = studioResources.referenceWorldbooks.find(item => item.id === bookToggle.dataset.referenceBookToggle);
            if (!book) return;
            referenceWorldbookProjectState(book, project, true).enabled = bookToggle.checked;
        ...
        const entryToggle = event.target.closest('[data-reference-entry-toggle]');
        if (entryToggle) {
            const book = studioResources.referenceWorldbooks.find(item => item.id === entryToggle.dataset.referenceEntryBook);
            if (!book) return;
            referenceWorldbookProjectState(book, project, true).entries[entryToggle.dataset.referenceEntryToggle] = entryToggle.checked;
            saveProject();
            renderReferenceWorldbooks();
            renderCurrentStep();
        }
...
async function importPresetFile(event) {
    ...
    try {
        ...
        if (!modelParameterSettings.customized) {
...
function bindStudioEvents() {
    ...
    artifactList.addEventListener('click', event => {
        ...
        const editManualButton = event.target.closest('[data-edit-manual-artifact]');
        if (editManualButton) {
            openManualArtifactDialog(editManualButton.dataset.editManualArtifact);
            return;
        }
...
.acs-assistant-config { overflow:auto; padding:24px; }
.acs-assistant-config-intro { margin-bottom:18px; }
.acs-assistant-config-intro h3 { margin:0 0 5px; font-size:19px; }
.acs-assistant-config-intro p { margin:0; color:#a79a8f; line-height:1.55; }
.acs-assistant-config-card { padding:18px; border:1px solid rgba(218,171,116,.26); border-radius:16px; background:linear-gradient(145deg,rgba(91,70,47,.18),rgba(255,255,255,.018)); }
.acs-assistant-config-section + .acs-assistant-config-section { margin-top:20px; padding-top:20px; border-top:1px solid rgba(255,255,255,.08); }
.acs-assistant-config-section-head { margin-bottom:13px; }
.acs-assistant-config-section-head strong { display:block; font-size:15px; }
.acs-assistant-config-section-head small { display:block; margin-top:4px; color:#9f9388; font-weight:400; }
.acs-assistant-config-grid { display:grid; grid-template-columns:minmax(180px,.65fr) minmax(0,1.35fr); gap:16px; }
...
.acs-assistant-knowledge-count { flex:0 0 auto; min-width:28px; padding:4px 8px; border-radius:999px; background:rgba(219,124,83,.15); color:#dda07f; text-align:center; font:700 11px/1.4 ui-monospace,monospace; }
.acs-assistant-knowledge-form { display:grid; grid-template-columns:minmax(0,1fr) 132px; gap:13px; }
...
.acs-assistant-knowledge-form textarea { min-height:190px; resize:vertical; font-weight:400; line-height:1.55; }
...
.acs-assistant-knowledge-editor-controls { display:flex; flex:0 0 auto; align-items:center; gap:9px; }
.acs-assistant-knowledge-delete { min-height:36px; padding:0 11px; border-color:rgba(217,132,127,.42); background:rgba(217,132,127,.08); color:#e2aaa5; }
.acs-assistant-knowledge-delete:hover:not(:disabled) { border-color:rgba(217,132,127,.72); color:#f0bbb6; }
@media (max-width:720px) { .acs-assistant-knowledge-editor-head { flex-wrap:wrap; } .acs-assistant-knowledge-editor-controls { width:100%; justify-content:space-between; } }
...
function populateCreativeAssistantKnowledgeEditor(item = null) {
    ...
    form.querySelector('[data-assistant-enabled-toggle]').setAttribute('aria-pressed', String(item?.enabled !== false));
...
function installCreativeAssistantUI() {
        ...
        <div class="acs-assistant-body">
            ...
            <section class="acs-assistant-panel acs-assistant-knowledge" data-assistant-panel="knowledge" hidden>
                ...
                <div class="acs-assistant-knowledge-workspace">
                    ...
                    <section class="acs-assistant-knowledge-editor"><form id="acs-assistant-knowledge-form" class="acs-assistant-knowledge-form"><div class="acs-assistant-knowledge-editor-head"><div><h4 id="acs-assistant-knowledge-editor-title">新建知识条目</h4><p id="acs-assistant-knowledge-source" hidden></p></div><input id="acs-assistant-knowledge-enabled" type="hidden" value="true"><div class="acs-assistant-knowledge-editor-controls"><button id="acs-assistant-knowledge-delete" class="acs-button acs-assistant-knowledge-delete" type="button" hidden><i class="fa-regular fa-trash-can"></i> 删除条目</button><button class="acs-assistant-enabled-toggle" type="button" data-assistant-enabled-toggle aria-pressed="true"><span class="acs-assistant-toggle-track" aria-hidden="true"><span></span></span><span>启用条目</span></button></div></div><label><span>标题</span><input id="acs-assistant-knowledge-title" type="text" maxlength="120" required placeholder="例如：世界观核心规则"></label><div class="acs-assistant-field"><span>作用范围</span><input id="acs-assistant-knowledge-scope" type="hidden" value="project"><div class="acs-assistant-segmented" role="group" aria-label="知识条目作用范围"><button type="button" data-assistant-scope="project" aria-pressed="true">当前项目</button><button type="button" data-assistant-scope="global" aria-pressed="false">全局</button></div></div><label class="is-wide"><span>条目内容</span><textarea id="acs-assistant-knowledge-content" maxlength="30000" required placeholder="填写要提供给创作助手的知识内容。"></textarea></label><div class="acs-assistant-knowledge-form-actions"><div class="acs-assistant-knowledge-form-actions-right"><button class="acs-button" type="button" data-assistant-knowledge-new>清空编辑器</button><button id="acs-assistant-knowledge-save" class="acs-button acs-button-primary" type="submit"><i class="fa-solid fa-plus"></i> 创建条目</button></div></div></form></section>
                </div>
            </section>
    ...
    overlay.addEventListener('click', event => {
        ...
        const enabledButton = event.target.closest('[data-assistant-enabled-toggle]');
        if (enabledButton) { const input = overlay.querySelector('#acs-assistant-knowledge-enabled'); input.checked = !input.checked; enabledButton.setAttribute('aria-pressed', String(input.checked)); const editingId = overlay.querySelector('#acs-assistant-knowledge-form').dataset.editingId; const item = creativeAssistantSettings.knowledgeEntries.find(entry => entry.id === editingId); if (item) { item.enabled = input.checked; item.updatedAt = new Date().toISOString(); saveCreativeAssistantSettings(); renderCreativeAssistantKnowledge(); } return; }
...
.acs-cloud-list-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:14px}.acs-cloud-list-toolbar small{color:#9c958b}.acs-cloud-refresh{display:inline-flex;align-items:center;gap:7px;min-height:36px;padding:0 12px;border:1px solid #5c554b;border-radius:10px;background:#3a362f;color:#ddd5cc}.acs-cloud-refresh:hover{border-color:#d27b5c;color:#fff}
.acs-cloud-body{min-height:0;overflow:auto;padding:22px 26px 28px}.acs-cloud-panel:not([hidden]){display:flex;min-height:100%;flex-direction:column}.acs-cloud-panel[hidden]{display:none}.acs-cloud-cards:has(>.acs-cloud-empty){flex:1}.acs-cloud-empty{display:grid;grid-column:1/-1;place-items:center;width:min(520px,100%);min-height:220px;margin:auto;padding:28px;border:1px solid #504a42;border-radius:18px;background:#302d27;box-shadow:inset 0 1px 0 #ffffff08;text-align:center;color:#aaa39a}.acs-cloud-empty p{margin:0}.acs-cloud-empty i{display:block;margin-bottom:14px;color:#856f9f;font-size:34px}
...
async function renderCloudRepository() {
    ...
    shell.querySelector('#acs-cloud-refresh-status').textContent = `${cloudSettings.owner}/${cloudSettings.repo}@${cloudSettings.branch || 'main'} · 已刷新 ${new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}`;
...
function installCloudRepositoryUI() {
    ...
    overlay.innerHTML = `<section class="acs-cloud-dialog" role="dialog" aria-modal="true" aria-labelledby="acs-cloud-title"><header class="acs-cloud-head"><div><p>CARD CLOUD</p><h2 id="acs-cloud-title">角色卡云仓库</h2></div><button class="acs-cloud-close" type="button" data-cloud-close aria-label="关闭"><i class="fa-solid fa-xmark"></i></button></header><nav class="acs-cloud-tabs"><button class="acs-cloud-tab is-active" type="button" data-cloud-tab="remote">云端角色卡</button><button class="acs-cloud-tab" type="button" data-cloud-tab="local">上传本地卡</button><button class="acs-cloud-tab" type="button" data-cloud-tab="connect">连接设置</button></nav><div class="acs-cloud-body">
      <section class="acs-cloud-panel" data-cloud-panel="remote"><div class="acs-cloud-list-toolbar"><small id="acs-cloud-refresh-status">尚未刷新</small><button id="acs-cloud-refresh" class="acs-cloud-refresh" type="button"><i class="fa-solid fa-rotate" aria-hidden="true"></i><span>刷新云端</span></button></div><div id="acs-cloud-remote-list" class="acs-cloud-cards"></div></section>
    ...
    overlay.addEventListener('click', async event => {
        ...
        if (tab) { for (const item of overlay.querySelectorAll('[data-cloud-tab]')) item.classList.toggle('is-active', item === tab); for (const panel of overlay.querySelectorAll('[data-cloud-panel]')) panel.hidden = panel.dataset.cloudPanel !== tab.dataset.cloudTab; if (tab.dataset.cloudTab === 'remote') { cloudRegistry = null; await renderCloudRepository().catch(error => notify('error', error.message)); } return; }
        ...
        if (pull) { const card = pull.closest('.acs-cloud-card'); const status = card?.querySelector('small'); const original = status?.textContent || ''; pull.disabled = true; pull.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>'; if (status) { status.textContent = '正在下载并导入…'; status.classList.add('acs-cloud-operation'); } try { const item = (await loadCloudRegistry()).cards.find(entry => entry.id === pull.dataset.cloudPull); if (!item) throw new Error('云端角色卡已不存在，请刷新后重试。'); await importCloudCharacter(item); if (status) { status.textContent = '已导入为新角色'; status.classList.add('is-success'); } } catch (error) { if (status) { status.textContent = `下载失败：${error.message}`; status.classList.add('is-error'); } notify('error', error.message); } finally { pull.disabled = false; pull.innerHTML = '<i class="fa-solid fa-cloud-arrow-down"></i>'; if (status && !status.classList.contains('is-success') && !status.classList.contains('is-error')) status.textContent = original; } return; }
        const archive = event.target.closest('[data-cloud-archive]');
        if (archive) { const card = archive.closest('.acs-cloud-card'); const status = card?.querySelector('small'); try { const item = (await loadCloudRegistry()).cards.find(entry => entry.id === archive.dataset.cloudArchive); if (!item) throw new Error('云端角色卡已不存在，请刷新后重试。'); if (!await showStudioConfirm({ title: '归档云端角色卡？', message: `“${item.name}”会从默认列表隐藏，但 Git 历史仍可恢复。`, confirmLabel: '归档' })) return; archive.disabled = true; archive.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>'; if (status) { status.textContent = '正在归档…'; status.classList.add('acs-cloud-operation'); } await archiveCloudCharacter(item); notify('success', `已归档“${item.name}”。`); await renderCloudRepository(); } catch (error) { if (status) { status.textContent = `归档失败：${error.message}`; status.classList.add('acs-cloud-operation', 'is-error'); } notify('error', error.message); } finally { if (archive.isConnected) { archive.disabled = false; archive.innerHTML = '<i class="fa-solid fa-box-archive"></i>'; } } return; }
    });
    ...
    overlay.querySelector('#acs-cloud-refresh').addEventListener('click', async event => { const refresh = event.currentTarget; refresh.disabled = true; refresh.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i><span>正在刷新</span>'; cloudRegistry = null; try { await renderCloudRepository(); notify('success', '云端角色卡已刷新。'); } catch (error) { notify('error', `刷新失败：${error.message}`); } finally { refresh.disabled = false; refresh.innerHTML = '<i class="fa-solid fa-rotate"></i><span>刷新云端</span>'; } });
