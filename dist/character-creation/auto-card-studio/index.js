// A.U.T.O 角色卡创作台 v0.5.9 · 酒馆助手脚本核心包（内置自动更新器）
...
const AUTO_CARD_STUDIO_VERSION = '0.5.9';
...
function collectArtifactGroups() {
    ...
    for (const step of STEPS) {
        ...
        for (const { turn, turnIndex, archived } of sources) {
            ...
            extractArtifactBlocks(turn.content, step.number).forEach((block, blockIndex) => {
                // 同名代码类型可能在不同步骤代表不同产物，历史版本只在当前步骤内合并。
                const groupKey = `${step.number}:${block.tag}`;
                if (!groups.has(groupKey)) groups.set(groupKey, { tag: block.tag, versions: [] });
...
function effectiveStepResponse(stepNumber) {
    ...
    if (!latest) return '';
    const latestTags = new Set(extractArtifactBlocks(latest, stepNumber).map(block => block.tag));
    const latestArtifacts = new Map();
    for (const turn of allAssistantArtifactTurns(stepNumber)) {
        for (const block of extractArtifactBlocks(turn.content, stepNumber)) latestArtifacts.set(block.tag, block.content);
    }
