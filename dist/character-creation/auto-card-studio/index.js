// A.U.T.O 角色卡创作台 v0.5.11 · 酒馆助手脚本核心包（内置自动更新器）
...
.acs-prompt-message summary::-webkit-details-marker {
  display: none;
}
...
.acs-prompt-message[open] summary i {
  transform: rotate(0deg);
}

.acs-prompt-message pre {
  max-height: 52vh;
  margin: 0;
  overflow: auto;
  padding: 15px 16px;
...
const PROMPT_PREVIEW_HTML = `
  <div id="acs-prompt-preview" class="acs-prompt-preview" aria-hidden="true" hidden>
    ...
    <section class="acs-prompt-preview-panel" role="dialog" aria-modal="true" aria-labelledby="acs-prompt-preview-title">
      ...
      <p class="acs-prompt-preview-note">这里按照实际发送顺序展示。为便于阅读，受保护的角色卡变量显示为 {{char}} 与 {{user}}；发送时会使用内部保护标记，防止被当前聊天环境提前替换。</p>
...
const AUTO_CARD_STUDIO_VERSION = '0.5.11';
...
const TOUR_STEPS = Object.freeze([
    {
        selector: '.acs-project-identity',
        placement: 'right',
        eyebrow: 'PROJECT 01',
        title: '从一个项目开始',
        description: '点击文件夹可以新建、切换或删除多个项目；项目名、创作母题和每一步的内容都会自动保存在浏览器中。',
    },
    {
        ...
        eyebrow: 'ROUTE 02',
        title: '沿着 30 个步骤推进',
        description: '四个大类可以折叠。你可以随时切换步骤，不必严格按顺序一次做完，已有内容也不会丢失。',
    },
    {
        ...
        eyebrow: 'STATION 03',
        title: '先看清当前任务',
        description: '中间区域会随左侧步骤变化。标题说明这一站要解决的问题，创作概览可以随时收起，为对话腾出空间。',
    },
    {
        selector: '.acs-composer',
        placement: 'top',
        eyebrow: 'DIALOGUE 04',
        title: '告诉 A.U.T.O 你的想法',
        description: '可以直接生成，也可以补充偏好、修改方向和限制。对结果不满意时，使用回复旁的重试按钮重新生成。',
    },
    {
        selector: '.acs-inspector',
        fallbackSelector: '#acs-inspector-toggle',
        placement: 'left',
        eyebrow: 'ARTIFACT 05',
        title: '在右侧整理创作产物',
        description: '产物页会提取结构化区块，可展开、编辑和复制；设置页管理模型连接，发布页负责最终交付。小屏幕可点顶部的分栏图标打开。',
    },
    {
        ...
        eyebrow: 'CONTROL 06',
        title: '最后记住这些工具',
        description: '这里可以检查脚本更新、导出项目、打开检查器或关闭创作台。项目平时会自动保存，导出文件适合备份和迁移。',
    },
]);
...
const STEP_ARTIFACT_RULES = Object.freeze({
    ...
    5: { prefixes: ['WORLD_main_characters_'] },
...
function buildProjectContext(currentStep, preset) {
    ...
    for (const step of STEPS) {
        ...
        const response = effectiveStepResponse(step.number);
...
function buildOrderedPrompts(preset, currentStep, options = {}) {
    ...
    const projectContext = { role: 'user', content: protectTemplateMacros(buildProjectContext(currentStep, preset)) };
    if (includePreviewMetadata) {
        macroGuard.name = '角色卡模板变量保护';
        projectContext.name = '项目上下文（母题、前序产物与本阶段对话）';
    }
    ...
    if (includePreviewMetadata) {
        ordered.push({
            role: 'user',
            name: '本轮输入',
            content: protectTemplateMacros(options.previewUserInput),
        });
    } else {
...
function renderPromptPreview(messages, step) {
    ...
    shell.querySelector('#acs-prompt-preview-summary').textContent = `Step ${step.number} · ${messages.length} 条消息 · ${totalCharacters.toLocaleString()} 字符 · ${connectionDisplayName()}`;

    messages.forEach((message, index) => {
        const details = document.createElement('details');
        details.className = 'acs-prompt-message';
        details.dataset.role = message.role || 'system';
        details.open = index === messages.length - 1 || message.name === `Step${step.number} ${step.name}`;
        ...
        size.textContent = `${String(message.content || '').length.toLocaleString()} 字符`;
        const icon = document.createElement('i');
        icon.className = 'fa-solid fa-chevron-down';
        icon.setAttribute('aria-hidden', 'true');
        summary.append(sequence, role, name, size, icon);

        const content = document.createElement('pre');
        content.textContent = restoreTemplateMacros(message.content);
...
async function runStepGeneration(step, state, userInput, { appendUserTurn = true, retried = false } = {}) {
    ...
    try {
        ...
        const result = await helper.generateRaw({
            ...
            user_input: protectTemplateMacros(userInput),
            should_stream: false,
...
function effectiveStepResponse(stepNumber) {
    const latest = latestAssistantResponse(stepNumber);
    if (!latest) return '';
    const latestBlocks = extractArtifactBlocks(latest, stepNumber);
    const latestTags = new Set(latestBlocks.map(block => resolveArtifactIdentity(stepNumber, block, latestBlocks)));
    ...
    const missing = [...latestArtifacts.entries()]
        .filter(([tag]) => !latestTags.has(tag))
        .map(([, content]) => content);
    return missing.length ? `${latest}\n\n${missing.join('\n\n')}` : latest;
}
...
function extractOpeningMessage() {
    const response = effectiveStepResponse(30);
