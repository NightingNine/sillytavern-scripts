...
| 步骤说明 | 通过。移除无法可靠关闭的 `<details>` 浮层，恢复原脚本的步骤编号、标题、引导语和“完成建议 / 建议怎么做 / 本步最终产物 / 教程提醒”四卡结构，并完整复用 29 步教程内容。“必做”徽标位于“完成建议”标题右侧，说明正文从下一行左侧起排。移动端改为全屏说明面板和顶部固定关闭键；MuMu 实测右上角 X 与 Android 返回键均可关闭，自动化测试同时覆盖遮罩和 Escape。 |
| 提示词预览 | 通过。不再复用可编辑 `EDITOR` 弹窗，恢复原脚本 `REQUEST MANIFEST / 本轮发送内容` 面板、统计摘要、说明条、复制与关闭按钮，以及按实际发送顺序排列的角色色标折叠消息卡片。MuMu 实测卡片展开后顶部标题和操作按钮保持固定，右上角 X 与 Android 返回键均可关闭。 |
...
- `ui-regression-step-guide-requirement-inline-final-20260724.png`
- `ui-regression-prompt-preview-final-20260724.png`
- `ui-regression-prompt-preview-expanded-stable-final-20260724.png`
- `ui-regression-prompt-preview-closed-final-20260724.png`
- `ui-regression-prompt-preview-back-closed-final-20260724.png`
...
- SHA-256：`D3BC146A78BA1F064B99E658BE95304748F4E8D249DA0441B50708D650940509`
...
- 最终界面截图：`.gstack/qa-reports/screenshots/ui-regression-prompt-preview-expanded-stable-final-20260724.png`
